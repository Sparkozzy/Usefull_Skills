...
graph TD
    ...
    Retell -->|Webhook Pós-Chamada| PCP_POST[post_call_processing]
    PCP_POST -->|RAG Master + Agente LLM| PCP_POST
    PCP_POST -->|Decisão Ligar| PCP[pre_call_processing]
    PCP_POST -->|Se Call_predict Ativo| CP[call_predict]

    Trigger[Gatilho / CRM] -->|Post Lead| CP
    CP -->|Lead Scoring & Timing Predict| PCP
...
| **`post_call_processing`** | `https://post-call-processing-api.bkpxmb.easypanel.host` | Processamento assíncrono pós-chamada, RAG no Supabase Master, Agente de IA (`gpt-4.1-nano`), roteamento Call_predict e retentativas via `pre_call_processing`. | [API Reference](file:///home/ryanf/Downloads/mind-all/Usefull_Skills/.agents/Skills/mindflow-services/post_call_processing/api_reference.md)<br>[Workflows e Arquitetura](file:///home/ryanf/Downloads/mind-all/Usefull_Skills/.agents/Skills/mindflow-services/post_call_processing/workflows_and_architecture.md) |
